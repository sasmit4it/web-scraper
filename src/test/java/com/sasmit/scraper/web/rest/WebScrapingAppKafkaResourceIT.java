package com.sasmit.scraper.web.rest;

import com.sasmit.scraper.config.KafkaProperties;
...
class WebScrapingAppKafkaResourceIT {
    ...
    void setup() {
        ...
        WebScrapingAppKafkaResource kafkaResource = new WebScrapingAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/web-scraping-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/web-scraping-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
